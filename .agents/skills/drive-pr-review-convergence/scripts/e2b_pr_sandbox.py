...
def provision(
    ...
    custom_setup = Path(setup_file).resolve()
    if not custom_setup.is_file():
    ...
    try:
        ...
        if bundle_path is not None:
            ...
            sandbox.files.write(remote_bundle, bundle_path.read_bytes())
            clone_source = remote_bundle
        else:
            clone_source = f"https://github.com/{pull['repo']}.git"
        ...
        record["setup"] = {
            "mode": "bundle" if bundle_path is not None else "file",
...
def command_create(args: argparse.Namespace) -> None:
    record = provision(
        ...
        args.setup_file,
        args.keep_running,
        getattr(args, "source_bundle", None),
        getattr(args, "source_repo_root", None),
    )
...
def command_harvest(args: argparse.Namespace) -> None:
    ...
    # git bundle verify/fetch/rev-parse need a repository with the bundle's
    # prerequisite objects; a bare rendered-package directory is not one.
    verify_cwd = Path(record.get("source_repo_root") or ROOT)
    host_command(
        ["git", "bundle", "verify", str(artifact)], cwd=verify_cwd
    )
    ...
    if not args.no_import:
        ...
        host_command(
            ["git", "fetch", str(artifact), f"HEAD:{imported_ref}"], cwd=verify_cwd
        )
        imported_sha = host_command(
            ["git", "rev-parse", imported_ref], cwd=verify_cwd
        ).stdout.strip()
...
def build_parser() -> argparse.ArgumentParser:
    ...
    create.add_argument(
        "--source-bundle",
        help="host-created git bundle for private repos; sandbox clones it without credentials",
    )
    create.add_argument(
        "--source-repo-root",
        help="git repository root used by harvest verification and the head-checked push",
    )
