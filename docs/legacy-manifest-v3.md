...
Surveyed 2026-08-02 by reading all 36 manifests and listing every object.
...
| **merged-only** | 9 | 3 | `stems.json`, `video.mp4`, `stems/stems_merged.webm` — **the six `.m4a` stems the manifest references do not exist** |
| **v2-era wav** | 1 (`the-pot-2d88b7a5`) | 8 | `stems.json` (no `version` key), `video.mp4`, `stems/{vocals,drums,bass,guitar,piano,shizzle}.wav` |

The 9 **merged-only** folders are broken as stem tracks: their `stems.json`
declares `stems/*.m4a` files that were never uploaded (or were deleted). Only
the pre-mixed `stems_merged.webm` and the video survive. Eight of the nine
duplicate a complete-v3 folder (same title, same duration); one
(`de330790cdfa`, "AC DC - Highway to Hell") and the Dirty Deeds /
For-Those-About-To-Rock pair have no complete twin, so that content exists only
as a merged mix.

Affected folders: `1de757812a7b`, `335561a490d4`, `3ae816991eb4`,
`4838f621a4e2`, `757be985b2f9`, `87eedd1c4ec2`, `8fa8cb05e8be`, `cdace71ff551`,
`de330790cdfa`.

## 2. Legacy schema

35 of 36 manifests carry exactly these top-level keys:
...
|---|---|---|---|
| `version` | `3` (35), absent (1) | `3` | force `3` |
| `title` | present, real | present | preserved verbatim |
| `artist` | `""` on all 35 v3 folders; `"TOOL"` on the v2-era one | present | preserved verbatim (mostly empty) |
| `duration` | real ffprobe seconds | same | preserved verbatim |
| `video` | `"video.mp4"` | `"video.mp4"` | unchanged |
| `timeline.sample_rate_hz` | `48000` on all 35 | **stem/model rate** (44100 for `htdemucs_6s`) | preserved verbatim; see §4 |
| `timeline.start_ms` / `duration_ms` | present | present | preserved verbatim |
| `video_meta` | present (35), absent (1) | present | preserved; `{}` when absent |
| `stems[].id` | already UI `StemId` incl. `shizzle` | same | unchanged |
| `stems[].name` | `"Vocals"` (35) / `"vocals"` (1) | `"Vocals"` | title-cased for the v2-era one |
| `stems[].file` | `stems/other.m4a` for id `shizzle` (35); `stems/shizzle.wav` (1) | `stems/other.m4a` | unchanged (files are copied under their existing names) |
| **`stems[].default_gain`** | **`0` (35) / `1.0` (1), linear** | — removed — | **dropped** |
...
| `sourceUrl`, `videoId` | absent (35), `""` (1) | present, may be empty | emitted as `""` |
...
of the 35 legacy v3 manifests carries `default_gain: 0`.** Read as the linear
...
Legacy declares `48000` on all 35 v3 folders; the new worker writes the Demucs
