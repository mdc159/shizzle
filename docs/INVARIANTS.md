...
update this file AND its guarding test in the same PR. `Where:` identifies
the implementation files; named guarding tests provide stable lookup anchors.
...
- Where: `interfaces/lossless-stem-v1/spec.md`, `stemsplit/lossless_handler.py`, `library/src/shizzle_server/publish/lossless_intake.py`
...
accepted.
- Where: `interfaces/lossless-stem-v1/spec.md`, `stemsplit/lossless_worker.py`, `library/src/shizzle_server/publish/delivery_profile.py`, `library/src/shizzle_server/publish/lossless_intake.py`
...
- Where: `interfaces/lossless-stem-v1/spec.md`, `stemsplit/lossless_worker.py`, `library/src/shizzle_server/publish/lossless_intake.py`
...
- Where: `library/src/shizzle_server/publish/lossless_intake.py`
...
- Where: `stemsplit/lossless_handler.py`, `stemsplit/lossless_worker.py`
...
- Where: `interfaces/lossless-stem-v1/spec.md`, `stemsplit/lossless_worker.py`, `stemsplit/Dockerfile.lossless`
...
non-worker callers append unfenced. `owns_lease` exposes the owner-plus-
...
forbidden.
Reusing a recovery UUID is refused and every operator process gets a fresh
lease-owner nonce. Later verification/publication failures retain that checked
recovery provenance, permitting another explicit recovery after its cause is
fixed without resetting the dispatch attempt counter. An already-published
generation can activate only when its immutable manifest equals the rebuilt
candidate; mismatches fail closed for reconciliation.
...
dispatch.
- Where: `library/src/shizzle_server/db/repository.py`, `library/src/shizzle_server/orchestrator/stages.py`
...
- Where: `library/src/shizzle_server/publish/publisher.py`, `docs/architecture.md`
...
- Where: `library/src/shizzle_server/publish/publisher.py`
...
- Where: `library/src/shizzle_server/orchestrator/cloud.py`, `library/src/shizzle_server/publish/lossless_intake.py`, `library/src/shizzle_server/db/repository.py`, `docs/architecture.md`
...
**Invariant:** A track row MUST be created only with a non-empty canonical
`s3_prefix` under `tracks/` (no empty segments or trailing slash) and the
`manifest_key` equal to `<s3_prefix>/manifest.json`; the repository refuses
anything else, recording a `publish_refused` job event (at most once per job
— the refusal is deterministic and the check-and-insert is serialized on the
job row FOR UPDATE) instead of publishing. The stub test pipeline MUST
require the explicit opt-in `SHIZZLE_ALLOW_TEST_PIPELINE` (default off), and
`/api/library` MUST never list a row outside `tracks/`.
- Where: `library/src/shizzle_server/db/repository.py`, `library/src/shizzle_server/orchestrator/pipelines.py`, `library/src/shizzle_server/settings.py`
- Guarded by: `library/tests/test_publish_guard.py::test_2026_08_19_phantom_replay_fails_closed`, `library/tests/test_publish_guard.py::test_publish_track_refuses_local_prefix_and_records_event`, `library/tests/test_publish_guard.py::test_publish_track_refuses_non_manifest_key`, `library/tests/test_publish_guard.py::test_publish_track_refuses_bare_tracks_prefix`, `library/tests/test_publish_guard.py::test_publish_track_refuses_manifest_outside_prefix`, `library/tests/test_publish_guard.py::test_publish_track_refusal_is_recorded_once`, `library/tests/test_publish_guard.py::test_publish_track_refusal_locks_job_row_before_event_check`, `library/tests/test_publish_guard.py::test_list_tracks_never_lists_non_tracks_prefix`, `library/tests/test_publish_guard.py::test_build_pipeline_refuses_test_pipeline_without_opt_in`, `library/tests/contract/test_orchestrator_postgres.py::TestConcurrentRefusal`
...
- Where: `library/src/shizzle_server/publish/delivery_profile.py`, `library/src/shizzle_server/publish/lossless_intake.py`
...
- Where: `library/src/shizzle_server/publish/delivery_profile.py`
...
- Where: `library/src/shizzle_server/api/models.py`, `library/src/shizzle_server/api/routes.py`
...
- Where: `library/src/shizzle_server/api/models.py`, `library/src/shizzle_server/db/models.py`
...
- Where: `docs/HANDOFF.md`, `deploy/vps/compose.prod.yml` (CloudFront key mounted read-only)
...
- Where: `library/src/shizzle_server/api/auth.py`, `library/src/shizzle_server/settings.py`
...
- Where: `library/src/shizzle_server/api/cloudfront.py`
- Guarded by: `library/tests/test_auth_media.py`
...
- Where: `library/tests/contract/conftest.py`
