...
- `timeline` has **exactly** the keys `start_ms`, `duration_ms`,
  `sample_rate_hz` — no extras — with values `start_ms: 0`,
  `duration_ms: round(duration*1000)` (tolerated within ±1 ms), and
...
the media (skipping objects whose remote size already matches, so an
interrupted drop can be restarted by re-running), and **always uploads
`manifest.json` last and unconditionally** — the completion marker is never
sent through the size-match skip path. With `--wait` it polls for the
...
```bash
# media first, any order — Content-Type matters
aws s3 cp DIR/stems/           s3://karaoke-pimpshizzle/imports/REF/stems/   --recursive --content-type audio/mp4
...
`would-publish` result without copying or registering anything. Exit codes:
`0` published / already-published / would-publish, `2` rejected,
`3` not ready (no manifest yet).
...
```json
{
  "status": "published | already-published | rejected | would-publish",
  "sourceRef": "youtube-…",
  "trackId": "uuid",
  "generation": 1,
...
| `TRACK_CONFLICT` | this ref already published different content; generation moves are not a drop-box operation |
...
A rejected drop is left exactly as uploaded (media, manifest, plus the new
`result.json`) — fix the package and drop again. Nothing was copied or
registered.
