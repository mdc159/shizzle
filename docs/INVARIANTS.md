...
**Invariant:** A track row MUST be created only with a non-empty `s3_prefix`
under `tracks/` and the `manifest_key` equal to `<s3_prefix>/manifest.json`;
the repository refuses anything else, recording a `publish_refused` job event
(at most once per job — the refusal is deterministic) instead of publishing.
The stub test pipeline MUST require the explicit opt-in
